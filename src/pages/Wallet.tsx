...
import { Wallet as WalletIcon, ArrowDownLeft, Copy, CreditCard, Building2, CheckCircle2, Eye, EyeOff, Upload, FileCheck2, Printer, Loader2, Zap, Lock, Unlock } from "lucide-react";
...
export default function Wallet() {
  ...
  useEffect(() => {
    ...
    loadCashback();
  }, [user?.id, wallet]); // re-load when wallet changes
    ...
    <div className="container py-10">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-8 bg-gradient-primary text-primary-foreground shadow-elevated overflow-hidden relative rounded-3xl">
          ...
          <div className="relative mt-8 grid grid-cols-3 gap-3 text-sm">
            ...
            <div className="p-3 rounded-xl bg-white/10">
              <p className="opacity-70 text-xs">Funded</p>
              <p className="font-semibold">
                ₦{user ? transactions.filter(t => t.type === "wallet").reduce((s, t) => s + t.amount, 0).toLocaleString() : "0"}
              </p>
            </div>
            <div className="p-3 rounded-xl bg-white/10">
              <p className="opacity-70 text-xs flex items-center gap-1">
                Cashback
                {hasFunded ? <Unlock className="h-3 w-3" /> : <Lock className="h-3 w-3" />}
              </p>
              <p className="font-semibold">
                {cashbackLoading ? "..." : hideBalance ? "₦••••" : `₦${cashbackBalance.toLocaleString()}`}
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6 shadow-card">
          <div className="flex items-center gap-3 mb-4">
            <div className={`h-10 w-10 rounded-xl grid place-items-center ${hasFunded ? "bg-emerald-500/15 text-emerald-600" : "bg-amber-500/15 text-amber-600"}`}>
              {hasFunded ? <Unlock className="h-5 w-5" /> : <Lock className="h-5 w-5" />}
            </div>
            <div>
              <h3 className="font-semibold">Cashback Balance</h3>
              <p className="text-xs text-muted-foreground">
                {hasFunded ? "Unlocked – ready to use" : "Locked until first funding"}
              </p>
            </div>
          </div>

          <p className="text-3xl font-bold tabular-nums mb-2">
            {cashbackLoading ? "..." : `₦${cashbackBalance.toLocaleString()}`}
          </p>

          {!hasFunded && cashbackBalance > 0 && (
            <div className="mt-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm">
              <p className="font-medium text-amber-700 dark:text-amber-400">Cashback is locked</p>
              <p className="text-xs text-muted-foreground mt-1">
                Fund your wallet at least once to unlock and use your cashback.
              </p>
            </div>
          )}
          ...
          {hasFunded && cashbackBalance > 0 && (
            <div className="mt-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-sm">
              <p className="font-medium text-emerald-700 dark:text-emerald-400">Cashback is unlocked</p>
              <p className="text-xs text-muted-foreground mt-1">
                You can now use cashback when buying data, airtime and other services.
              </p>
            </div>
          )}
          ...
          {cashbackBalance === 0 && (
            <p className="text-sm text-muted-foreground mt-3">
              Earn cashback by referring friends or receiving welcome bonus.
            </p>
          )}
        </Card>
      </div>

      {/* Rest of the funding cards remain the same */}
        ...
        {manualEnabled && (
          <Card className="p-6 shadow-card border-2 border-primary/20 hover-lift">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-lg">🏦 Fund via Bank Transfer</h3>
                <p className="text-xs text-muted-foreground">Upload receipt for review</p>
              </div>
            </div>
            ...
            <div className="space-y-2 text-sm bg-muted/40 rounded-xl p-4">
              ...
              <Row label="Account number">
                <span className="flex items-center gap-2 font-mono">
                  {activeBank?.account_number || settings.accountNumber}
                  <Copy className="h-3.5 w-3.5 cursor-pointer hover:text-primary" onClick={() => { navigator.clipboard.writeText(activeBank?.account_number || settings.accountNumber); toast.success("Copied"); }} />
                </span>
              </Row>
        ...
        {paystackEnabled && (
          <Card className="p-6 shadow-card border-2 border-emerald-500/20 hover-lift bg-gradient-to-br from-card to-emerald-500/5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-lg">💳 Fund via Paystack</h3>
            ...
            <Input
              ...
              value={psAmount}
              onChange={(e) => setPsAmount(e.target.value === "" ? "" : Number(e.target.value))}
            />
            <div className="grid grid-cols-4 gap-2 mt-3">
              {[1000, 2000, 5000, 10000].map((a) => (
                <button
                  ...
                  className={`text-xs rounded-lg py-2 border ${psAmount === a ? "border-emerald-500 bg-emerald-500/10" : "border-border hover:bg-muted"}`}
                >
                  ₦{a.toLocaleString()}
                </button>
              ))}
      ...
      <Card className="p-6 shadow-card mt-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h3 className="font-semibold">Fund Wallet (Manual)</h3>
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          {[500, 1000, 2000, 5000, 10000, 20000].map((a) => (
            <button
              ...
              type="button"
              onClick={() => setAmount(a)}
              className={`text-sm rounded-lg py-2 border ${amount === a ? "border-primary bg-accent" : "border-border hover:bg-muted"}`}
            >
              ₦{a.toLocaleString()}
            </button>
          ))}
      ...
      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setStep("idle"); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Your ₦{Number(amount).toLocaleString()} funding request was submitted for verification. You'll be notified once approved.</DialogDescription>
          </DialogHeader>
