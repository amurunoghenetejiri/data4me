  ...
  return (
    <div className="container py-10">
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {networks.map((n) => (
              <button
                ...
                disabled={processing}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition disabled:opacity-50 ${
                  network === n.id ? "border-primary bg-accent" : "border-transparent bg-muted/40 hover:bg-muted"
                }`.trim()}
              >
            ...
            {airtimeAmounts.map((a) => (
              <button
                ...
                className={`px-3 py-2 rounded-lg text-sm font-medium border transition disabled:opacity-50 ${
                  amount === a ? "border-primary bg-accent text-accent-foreground" : "border-border hover:bg-muted"
                }`.trim()}
              >
                ₦{a.toLocaleString()}
              </button>
      ...
      <PinDialog
        ...
        description={`Confirm ₦${amount.toLocaleString()} ${network.toUpperCase()} airtime to ${phone}.`}
      />
